...
from .compat import *
from .path import get_module_name, get_classname
...
#         m = loader.exec_module(loaded)
    ...
    for patch_name, patch in patches.items():
        if "." in patch_name:
            ...
            if sm:
                setattr(m, parts[0], patch_module(sm, {".".join(parts[1:]): patch}))
            else:
...
def mock_class(name="", patches=None, **kwargs_patches):
    ...
    if not patches: patches = {}
    patches.update(kwargs_patches)
    return type(get_classname(name=name), (Mock,), patches)


def mock_instance(name="", patches=None, **kwargs_patches):
    """This is the same as mock_class but returns an instance of that class

    see mock_class() docs"""
    return mock_class(name)(patches, **kwargs_patches)


def mock(patches=None, **kwargs_patches):
